...
def conn_to_messages_log(IP,TIME):
    up=False
    lst=[]
    tmp_lst=[]
    try:
        for n in range(0,5):
            with telnetlib.Telnet(IP) as t:
                ...
                log_file=f"show log messages.{n}.gz"
                print(log_file)
                t.write(log_file.encode(encoding="utf-8")+b"| match SNMP_TRAP | match "+TIME.encode(encoding="utf-8")+b'\n')
